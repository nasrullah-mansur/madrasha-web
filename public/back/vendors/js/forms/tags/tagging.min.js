...
!(function (a, b, c, d) {
    ...
    (e.prototype = {
        ...
        add: function (b) {
            var d, e, f, g, h, i, j;
            if (((f = this), a.isArray(b)))
                return a.each(b, function () {
                    f.add(this + "");
                });
            if (
                ((h = f.config["forbidden-words"]),
                b || ((b = f.valInput()), f.emptyInput()),
                !b || !b.length)
            )
                return !1;
            for (
                f.config["case-sensitive"] || (b = b.toLowerCase()),
                    e = h.length;
                e--;

            )
                if (((g = b.indexOf(h[e])), g >= 0))
                    return (
                        f.emptyInput(),
            ...
            return (
                (d = a(c.createElement("div"))
                    .addClass(f.config["tag-class"])
                    .html("<span>" + f.config["tag-char"] + "</span> " + b)),
                a(c.createElement("input"))
                    .attr("type", "hidden")
                    .attr("name", f.config["tags-input-name"] + "[]")
                    .val(b)
                    .appendTo(d),
                a(c.createElement("a"))
                    .attr("role", "button")
                    .addClass(f.config["close-class"])
                    .html(f.config["close-char"])
                    .click(function () {
                        f.remove(d);
                    })
                    .appendTo(d),
                ...
                f.$elem.trigger("add:after", [b, f]),
                !0
            );
        },
        addSpecialKeys: function (b) {
            var c, d, e, f, g;
            if (((c = this), (g = b[0]), (f = b[1]), (e = {}), a.isArray(f)))
        ...
        getTags: function () {
            var a, b, c;
            for (c = this.tags.length, a = [], b = 0; c > b; b += 1)
                a.push(this.tags[b].pure_text);
            return a;
        },
        getTagsObj: function () {
            return this.tags;
        ...
        init: function () {
            ...
            return (
                ...
                (b = d.$elem.text()),
                d.$elem.empty(),
                (d.$type_zone = a(c.createElement("input"))
                    .addClass(d.config["type-zone-class"])
                    .attr("contenteditable", !0)),
                d.$elem
                    .addClass(d.config["tag-box-class"])
                    .append(d.$type_zone),
                d.$type_zone.keydown(function (a) {
                    var b, c, e, f, g, h, i, j, k;
                    if (
                        ((g = d.getSpecialKeys()),
                        (h = d.config["forbidden-chars"]),
                        (i = d.valInput()),
                        (f = a.which),
                        i)
                    ) {
                        for (e = h.length; e--; )
                            if (((c = i.indexOf(h[e])), c >= 0))
                                return (
                                    a.preventDefault(),
                                    (i = i.replace(h[e], "")),
                                    d.focusInput(),
                                    d.valInput(i),
                                    (j = d.config["forbidden-chars-callback"]),
                                    (k = d.config["forbidden-chars-text"]),
                                    d.throwError(j, k, h[e])
                                );
                        for (b in d.keys.add)
                            if (f === d.keys.add[b] && !d.config["no-" + b])
                                return a.preventDefault(), d.add();
                    } else
                        for (b in g)
                            if (f === g[b]) {
                                if (d.keys.add[b])
                                    return a.preventDefault(), !0;
                                if (d.keys.remove[b] && !d.config["no-" + b])
                                    return a.preventDefault(), d.remove();
                            }
                    return !0;
                }),
                d.config["tag-on-blur"] &&
                    d.$type_zone.focusout(function () {
        ...
        refresh: function (b) {
            ...
            return (
                (c = this),
                (d = c.config["pre-tags-separator"]),
                (b = b || c.getTags().join(d)),
                c.reset(),
                a.each(b.split(d), function () {
                    c.add(this + "");
                }),
                !0
            );
        ...
        remove: function (b) {
            ...
            if (b)
                for (e = c.tags.length; e--; )
                    c.tags[e][0].innerHTML === b[0].innerHTML &&
                        c.tags.splice(e, 1);
            else b = c.tags.pop();
            return (
                (d = d || b.pure_text),
                b.remove(),
                c.config["edit-on-delete"] &&
                    (c.emptyInput(), c.valInput(b.pure_text)),
                c.$elem.trigger("remove:after", [d, c]),
                b
            );
        },
        removeAll: function () {
        ...
        removeSpecialKeys: function (b) {
            var c, e, f, g, h;
            if (((c = this), (h = b[0]), (g = b[1]), (f = {}), a.isArray(g)))
                return a.each(g, function () {
                    c.removeSpecialKeys([h, this]);
                });
            for (e in c.keys[h])
                c.keys[h].hasOwnProperty(e) &&
                    c.keys[h][e] === g &&
                    (c.keys[h][e] = d);
            return c.keys[h];
        },
